...
package com.dunnysoft.ogg.jorbis;

import com.dunnysoft.ogg.jogg.*;
...
public class Info{
  ...
  //PsyInfo[] psy_param=new PsyInfo[psys=64]; // encode only
	...
	public int bitrate()
	{
		...
		else
		{
			if (bitrate_upper > 0)
			{
				if (bitrate_lower > 0)
				{
					return (bitrate_upper + bitrate_lower) / 2;
				}
				else
				{
					return bitrate_upper;
				}
			}
		}
		return (-1);
