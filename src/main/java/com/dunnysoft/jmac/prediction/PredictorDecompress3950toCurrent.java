...
package com.dunnysoft.jmac.prediction;

import com.dunnysoft.jmac.info.CompressionLevel;
import com.dunnysoft.jmac.tools.JMACException;
import com.dunnysoft.jmac.tools.RollBufferFastInt;
...
//        ai3[indexB] = m_Stage1FilterB.Compress(nB);
        final int k2 = indexB - 1;
        ai3[k2] = ai3[indexB] - ai3[k2];

        final int ai2[];
        final int ai4[];
        final int nPredictionA = (l * (ai2 = m_aryMA)[0]) + (ai[l1] * ai2[1]) + (ai[indexA - 2] * ai2[2]) + (ai[indexA - 3] * ai2[3]);
        final int nPredictionB = (ai3[indexB] * (ai4 = m_aryMB)[0]) + (ai3[k2] * ai4[1]) + (ai3[indexB - 2] * ai4[2]) + (ai3[indexB - 3] * ai4[3]) + (ai3[indexB - 4] * ai4[4]);

        final int nCurrentA = nA + ((nPredictionA + (nPredictionB >> 1)) >> 10);

        final RollBufferFastInt adaptA;
        final RollBufferFastInt adaptB;
        final int indexAA = (adaptA = m_rbAdaptA).index;
        final int indexAB = (adaptB = m_rbAdaptB).index;
        final int ai1[];
